...
public class RegisterServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		String role = request.getParameter("role"); System.out.println("submit role: " + role);
		String name = request.getParameter("name");
		String address = request.getParameter("address");
		String phone = request.getParameter("phone");
		
		// Kiểm tra sự phù hợp của dữ liệu nhập vào form
		if (!pwd.matches(regexPwd) || !usr.matches(regexMail)) {//-> register.jsp again nếu user&pass chưa phù hợp cú pháp
