	...
	<div class="register">
		<form class="form" id="form-2" name="f2" action="register" onsubmit="return validate()" method="post">
			<div class="form-group">
				...
				<select class="form-select" id="accrole" name="role" aria-label="Default select example">
					<option value="0" selected='selected'>account_role = 0</option>
					<option value="1" ${role == '1' ? 'selected' : ''}>account_role = 1</option>
					<option value="2" ${role == '2' ? 'selected' : ''}>account_role = 2</option>
				</select>
